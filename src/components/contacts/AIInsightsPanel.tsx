...
import { useIntelligenceEngine } from '../../hooks/useAdvancedAI';
...
import { SmartRecommendation } from '../../services/ai-intelligence-engine.service';
...
// Icon and color mappings - ensure keys match ContactInsight['type']
const insightIcons: Record<ContactInsight['type'], typeof Brain> = {
...
const insightColors: Record<ContactInsight['type'], string> = {
...
// Helper to safely access insight type
const getInsightIcon = (type: ContactInsight['type']) => insightIcons[type] || Lightbulb;
const getInsightColor = (type: ContactInsight['type']) => insightColors[type] || 'bg-gray-500';
const getImpactColor = (impact: 'high' | 'medium' | 'low') => impactColors[impact] || 'text-gray-600 bg-gray-50';
...
export const AIInsightsPanel: React.FC<AIInsightsPanelProps> = ({ contact }) => {
  ...
  const { 
    generateInsights: generateContactInsights, 
    scoreContact, 
    contactScore, 
    contactInsights, 
    isContactProcessing 
  ...
  // Load insights when component mounts - only once
  useEffect(() => {
  ...
  const handleGenerateInsights = async () => {
    ...
    try {
      ...
      await generateContactInsights(contact, ['opportunity', 'recommendation', 'risk', 'prediction']);
      setInsightsGenerated(true);

      researchThinking.complete('✅ Web-enhanced insights generated successfully!');

    } catch (error) {
      logger.error('AIInsightsPanel: Failed to generate insights', error as Error, { contactId: contact.id });
  ...
  const handleRefreshScore = async () => {
    try {
      ...
      logger.error('Failed to refresh score', error as Error, { contactId: contact.id });
    }
  };
  ...
  const handleGenerateIntelligence = async () => {
    try {
      ...
      logger.error('AIInsightsPanel: Failed to generate intelligence', error as Error, { contactId: contact.id });
    }
  };

  const handleFeedback = useCallback((insightId: string, feedback: 'positive' | 'negative') => {
    setFeedbackGiven(prev => ({ ...prev, [insightId]: feedback }));
    recordFeedback(insightId, feedback);
  }, [recordFeedback]);

  // Use real insights from AI Context
  const insights: ContactInsight[] = contactInsights || [];
  ...
  // Calculate average confidence across all insights, not filtered
  const avgConfidence = insights.length > 0 
    ? Math.round(insights.reduce((sum, insight) => sum + insight.confidence, 0) / insights.length)
    : 0;

  // Calculate high priority recommendations count
  ...
  const views = [
    ...
    { id: 'intelligence' as const, label: 'Intelligence Engine', icon: Brain },
    { id: 'recommendations' as const, label: 'Smart Recommendations', icon: Target }
  ];
  return (
    <>
                      ...
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          ...
                          <div className="flex items-center space-x-3 mt-1">
                            ...
                            {insight.actionable && (
                              <span className="px-2 py-1 bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300 rounded-md text-xs font-medium">
                                ACTIONABLE
                              </span>
                            )}
                          </div>
        ...
        <div className="space-y-4">
          {isIntelligenceAnalyzing ? (
            ...
            intelligence.map((intel) => (
              <GlassCard key={intel.id} className="p-6 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 border-blue-200 dark:border-blue-800">
                <div className="flex items-start space-x-4">
